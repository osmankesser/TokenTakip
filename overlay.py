...
from prompt_coach import CoachReport, Finding, build_report  # noqa: E402
from usage_client import Meter, ProviderUsage, UsageSnapshot, fetch_snapshot  # noqa: E402
...
_IDEA_GLYPH_PX = 26
_THEME_GLYPH_PX = 40
# Segoe Fluent (moon) + Symbol (diğerleri) — renk tile'dan gelir
_THEME_FLUENT = {"moon": 0xE708}
_THEME_SYMBOL = {"snow": "\u2744", "aurora": "\u2728", "flame": "\u2668"}
_FLUENT = {
    "kota": 0xE9D2,
    "ideas": 0xEA80,
    "chat": 0xE8BD,
...
def _cache_dir() -> Path:
    root = Path(os.environ.get("LOCALAPPDATA", str(ROOT))) / "Pulse" / "cache"
    path = root / "icons"
...
def _pix_from_cache(name: str, size: int) -> QPixmap | None:
    path = _cache_dir() / f"{name.lower().replace(' ', '_')}.png"
    if not path.is_file():
        return None
...
def _theme_glyph(kind: str, color: QColor, size: int = 40) -> QPixmap:
    """Tema kutusu — Windows Fluent + Symbol, tek renk."""
    if kind in _THEME_FLUENT:
        return _font_pix(chr(_THEME_FLUENT[kind]), color, size, ("Segoe Fluent Icons", "Segoe MDL2 Assets"), 0.76)
    sym = _THEME_SYMBOL.get(kind, "\u25cf")
    return _font_pix(sym, color, size, ("Segoe UI Symbol", "Segoe UI"), 0.82)


def _glyph(kind: str, color: QColor, size: int = 22, phase: float = 0.0, press: float = 0.0) -> QPixmap:
    if kind in _THEME_FLUENT or kind in _THEME_SYMBOL:
        return _theme_glyph(kind, color, size)
    cp = _FLUENT.get(kind)
    if cp is not None:
        return _font_pix(chr(cp), color, size, ("Segoe Fluent Icons", "Segoe MDL2 Assets"), 0.78)
    pix = QPixmap(size, size)
    pix.fill(Qt.transparent)
    p = QPainter(pix)
    p.setRenderHint(QPainter.Antialiasing)
...
def _flag_pix(lang: str, size: int = _FLAG_ICON_PX) -> QPixmap:
    path = _flag_cache_path(lang)
    if path.is_file():
        pix = QPixmap(str(path))
        if not pix.isNull():
            return pix.scaled(size, size, Qt.KeepAspectRatio, Qt.SmoothTransformation)
    return _letter_pix(lang.upper(), size)


def _save_flag_png(lang: str, data: bytes) -> QPixmap | None:
    pix = QPixmap()
    ...
    return pix.scaled(_FLAG_ICON_PX, _FLAG_ICON_PX, Qt.KeepAspectRatio, Qt.SmoothTransformation)


def _fetch_flags_sync(langs: list[str]) -> None:
    """Bayraklar yerel harf ikonu; dış ağ yok."""
    return
...
def _theme_css(theme: str) -> str:
    ...
    return f"""
QFrame#panel[theme="{t}"] QFrame#contentArea {{ background: {p["content"]}; }}
QFrame#panel[theme="{t}"] QFrame#headerLine {{ background: {p["line"]}; }}
QFrame#panel[theme="{t}"] QFrame#navBar {{ border-top: 1px solid {p["line"]}; }}
QFrame#panel[theme="{t}"] QLabel:not(#themeLbl) {{ color: {p["text"]}; }}
QFrame#panel[theme="{t}"] QLabel#welcomeTitle, QFrame#panel[theme="{t}"] QLabel#pageTitle,
QFrame#panel[theme="{t}"] QLabel#cardName,
QFrame#panel[theme="{t}"] QLabel#statN,
QFrame#panel[theme="{t}"] QLabel#ideaTitle {{ color: {p["title"]}; }}
QFrame#panel[theme="{t}"] QLabel#stamp, QFrame#panel[theme="{t}"] QLabel#section,
QFrame#panel[theme="{t}"] QLabel#meterName, QFrame#panel[theme="{t}"] QLabel#meterMeta,
QFrame#panel[theme="{t}"] QLabel#versionLabel, QFrame#panel[theme="{t}"] QLabel#ideaBody,
QFrame#panel[theme="{t}"] QLabel#statL, QFrame#panel[theme="{t}"] QLabel#ideaDate {{
    color: {p["muted"]};
}}
QFrame#panel[theme="{t}"] QCheckBox#switch::indicator:checked {{ background: {a}; }}
QFrame#panel[theme="{t}"] QLabel#error {{ color: {p["error"]}; }}
QFrame#panel[theme="{t}"] QLabel#ideaSource {{ color: {p["title"]}; }}
QFrame#panel[theme="{t}"] QLabel#meterValue, QFrame#panel[theme="{t}"] QLabel#leftPct,
QFrame#panel[theme="{t}"] QLabel#ideaFix {{ color: {p["success"]}; }}
QFrame#panel[theme="{t}"] QLabel#plan {{
    color: {a}; background: {p["accent_bg"]}; border: 1px solid {p["accent_border"]};
}}
QFrame#panel[theme="{t}"] QLabel#planLocal {{
    color: {p["text"]}; background: {p["segment"]}; border: 1px solid {p["card_border"]};
}}
QFrame#panel[theme="{t}"] QFrame#card, QFrame#panel[theme="{t}"] QFrame#statBox,
QFrame#panel[theme="{t}"] QFrame#settingRow {{
    background: {p["card"]}; border: 1px solid {p["card_border"]};
}}
QFrame#panel[theme="{t}"] QFrame#ideaCard {{
    background: {p["card"]}; border: 1px solid {p["card_border"]};
}}
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="warn"] {{
    background: {p["idea_warn_bg"]}; border-color: {p["idea_warn_border"]};
}}
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="info"] {{
    background: {p["idea_info_bg"]}; border-color: {p["idea_info_border"]};
}}
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="danger"] {{
    background: {p["idea_danger_bg"]}; border-color: {p["idea_danger_border"]};
}}
QFrame#panel[theme="{t}"] QPushButton#lookBtn {{
    background: {p["card"]}; color: {p["title"]}; border: 1px solid {p["field_border"]};
}}
QFrame#panel[theme="{t}"] QFrame#iconMark {{
    background: {p["icon_mark"]}; border: 1px solid {p["icon_mark_border"]};
}}
QFrame#panel[theme="{t}"] QFrame#segment {{ background: {p["segment"]}; }}
QFrame#panel[theme="{t}"] QPushButton#segBtn {{ color: {p["text"]}; }}
QFrame#panel[theme="{t}"] QPushButton#segBtn[active="true"] {{
    background: qlineargradient(x1:0,y1:0,x2:1,y2:0,stop:0 {ad}, stop:1 {ad});
    color: #ffffff;
}}
QFrame#panel[theme="{t}"] QComboBox {{
    background: {p["card"]}; color: {p["title"]}; border: 1px solid {p["field_border"]};
}}
QFrame#panel[theme="{t}"] QLabel#navText {{ color: {p["muted"]}; }}
QFrame#panel[theme="{t}"] QLabel#navText[active="true"] {{ color: {a}; font-weight: 800; }}
QFrame#panel[theme="{t}"] QLabel#navIcon[active="true"] {{ background: {p["accent_soft"]}; }}
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="warn"] QLabel#ideaBody,
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="warn"] QLabel#ideaTitle,
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="warn"] QLabel#ideaFix,
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="info"] QLabel#ideaBody,
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="info"] QLabel#ideaTitle,
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="info"] QLabel#ideaFix,
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="danger"] QLabel#ideaBody,
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="danger"] QLabel#ideaTitle,
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="danger"] QLabel#ideaFix,
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="danger"] QLabel#ideaSource {{
    color: {p["text"]};
}}
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="warn"] QLabel#ideaDate,
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="info"] QLabel#ideaDate,
QFrame#panel[theme="{t}"] QFrame#ideaCard[kind="danger"] QLabel#ideaDate {{
    color: {p["muted"]};
}}
QFrame#panel[theme="{t}"] QLabel#loadingLabel {{ color: {p["muted"]}; font-size: 11px; }}
QFrame#panel[theme="{t}"] QComboBox::down-arrow {{ border-top-color: {p["muted"]}; }}
QFrame#panel[theme="{t}"] QPushButton#roundBtn {{ background: transparent; border: none; }}
QFrame#panel[theme="{t}"] QFrame#themeTile[kind="night"] QLabel#themeLbl {{ color: {p["title"]}; }}
QFrame#panel[theme="{t}"] QFrame#themeTile[active="true"] {{ border: 2px solid {a}; }}
"""
...
QLabel#section { font-size: 12px; font-weight: 700; color: #64748b; margin-top: 2px; }
QLabel#plan {
    color: #2563eb; font-size: 10px; font-weight: 700;
    background: #eff6ff; border: 1px solid #dbeafe; border-radius: 10px; padding: 3px 8px;
}
...
QLabel#planLocal {
    ...
    background: #f1f5f9; border: 1px solid #e2e8f0; border-radius: 10px; padding: 3px 8px;
}
...
QFrame#statBox {
    background: #ffffff; border: 1px solid #e8ecf1; border-radius: 12px; min-height: 76px;
}
QFrame#settingRow {
    background: #ffffff; border: 1px solid #e8ecf1; border-radius: 12px;
}
QFrame#card, QFrame#ideaCard {
...
QFrame#segment {
    background: #eef2f6; border: none; border-radius: 10px;
}
QPushButton#segBtn {
...
QPushButton#themeTile, QToolButton#themeTile, QFrame#themeTile {
    border: 2px solid #e8ecf1; border-radius: 12px; min-height: 72px; font-weight: 700; font-size: 10px;
    background: #ffffff; color: #334155;
}
QLabel#themeLbl { font-size: 10px; font-weight: 700; background: transparent; color: inherit; }
QFrame#themeTile[kind="frost"] QLabel#themeLbl { color: #0c1929; }
QFrame#themeTile[kind="night"] QLabel#themeLbl { color: #f1f5f9; }
QFrame#themeTile[kind="aurora"] QLabel#themeLbl, QFrame#themeTile[kind="ember"] QLabel#themeLbl { color: #ffffff; }
QPushButton#themeTile[active="true"], QToolButton#themeTile[active="true"], QFrame#themeTile[active="true"] {
    border: 2px solid #2563eb;
}
QPushButton#themeTile[kind="night"], QToolButton#themeTile[kind="night"], QFrame#themeTile[kind="night"] {
    background: #1e293b; color: #f1f5f9;
}
QPushButton#themeTile[kind="frost"], QToolButton#themeTile[kind="frost"], QFrame#themeTile[kind="frost"] {
    background: qlineargradient(x1:0,y1:0,x2:1,y2:1,stop:0 #e0f2fe, stop:1 #dbeafe); color: #0c1929;
}
QPushButton#themeTile[kind="aurora"], QToolButton#themeTile[kind="aurora"], QFrame#themeTile[kind="aurora"] {
    background: qlineargradient(x1:0,y1:0,x2:1,y2:1,stop:0 #7c3aed, stop:1 #2563eb); color: #fff;
}
QPushButton#themeTile[kind="ember"], QToolButton#themeTile[kind="ember"], QFrame#themeTile[kind="ember"] {
    background: qlineargradient(x1:0,y1:0,x2:1,y2:1,stop:0 #ea580c, stop:1 #fbbf24); color: #fff;
}
...
QComboBox {
    ...
    border: 1px solid #cbd5e1; border-radius: 10px; padding: 8px; font-size: 12px;
}
QComboBox { min-height: 34px; padding: 6px 10px; }
...
class Bar(QWidget):
    def __init__(self, parent=None, tall: bool = False):
        super().__init__(parent)
        self._value = 0.0
        self.setFixedHeight(14 if tall else 10)
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        if isinstance(win, UsageOverlay):
            pal = THEME_PALETTE.get(win._theme, THEME_PALETTE["frost"])
        ...
        color = QColor("#22c55e") if self._value >= 40 else QColor("#eab308") if self._value >= 15 else QColor("#ef4444")
        fill = QPainterPath()
...
class ThemeTile(QFrame):
    def __init__(self, overlay: "UsageOverlay", name: str, glyph: str, color: QColor, parent=None):
        super().__init__(parent)
        self._overlay, self._name = overlay, name
        ...
        self.icon = QLabel()
        self.icon.setAlignment(Qt.AlignCenter)
        self.icon.setPixmap(_theme_glyph(glyph, color, _THEME_GLYPH_PX))
        self.lbl = QLabel()
        self.lbl.setObjectName("themeLbl")
    ...
    def click(self) -> None:
        self._overlay.set_theme(self._name)
...
class UsageOverlay(QWidget):
    def __init__(
        ...
        *,
        ...
        self._flag_workers: list[FlagBatchWorker] = []
        ...
        self._settings = QSettings("TokenTakip", "ui")
        if for_test:
        ...
        set_l = QVBoxLayout(settings_page)
        ...
        self.theme_btns = []
        theme_icons = {
            "night": ("moon", QColor("#e2e8f0")),
            "frost": ("snow", QColor("#0369a1")),
            "aurora": ("aurora", QColor("#ffffff")),
            "ember": ("flame", QColor("#fff7ed")),
        }
        for name in THEMES:
            glyph, col = theme_icons[name]
            tile = ThemeTile(self, name, glyph, col)
            self.theme_btns.append(tile)
            theme_row.addWidget(tile, 1)
        ...
        self.pin_label.setObjectName("section")
        self.pin_hint = QLabel()
        self.pin_hint.setObjectName("meterMeta")
        pin_frame = QFrame()
        pin_frame.setObjectName("settingRow")
        pin_row = QHBoxLayout(pin_frame)
        pin_row.setContentsMargins(12, 10, 12, 10)
        pin_left = QVBoxLayout()
        pin_left.setSpacing(2)
        ...
        self.boot_label.setObjectName("section")
        self.boot_hint = QLabel()
        self.boot_hint.setObjectName("meterMeta")
        boot_frame = QFrame()
        boot_frame.setObjectName("settingRow")
        boot_row = QHBoxLayout(boot_frame)
        boot_row.setContentsMargins(12, 10, 12, 10)
        boot_left = QVBoxLayout()
        boot_left.setSpacing(2)
        ...
        set_l.addStretch(1)
        set_l.addWidget(self.version_label)
        ...
        self.setStyleSheet(STYLE + _theme_css(self._theme))
    ...
    def _ensure_flags(self) -> None:
        missing = [code for code, _ in LANGS if not _flag_cache_path(code).is_file()]
        if not missing:
            self._refresh_lang_flag_icons()
            return
        worker = FlagBatchWorker(missing, self)
        worker.done.connect(self._got_flag)
        worker.finished.connect(self._refresh_lang_flag_icons)
        self._flag_workers.append(worker)
        worker.start()

    def _got_flag(self, lang: str, data: bytes) -> None:
        pix = _save_flag_png(lang, data)
        ...
        if idx >= 0:
            self.lang_combo.setItemIcon(idx, icon)
    ...
    def set_theme(self, name: str) -> None:
        ...
        self.setStyleSheet(STYLE + _theme_css(name))
        _NAV_ICON_CACHE.clear()
        self._refresh_theme_icons()
        for i, tname in enumerate(THEMES):
            self.theme_btns[i].set_active(tname == name)
        self._sync_nav()
    ...
    def _apply_language(self) -> None:
        ...
        self.filter_warn.setIconSize(QSize(_CHIP_GLYPH_PX, _CHIP_GLYPH_PX))
        self.filter_danger.setIconSize(QSize(_CHIP_GLYPH_PX, _CHIP_GLYPH_PX))
        self.filter_info.setIconSize(QSize(_CHIP_GLYPH_PX, _CHIP_GLYPH_PX))
        ...
        self.filter_all.setProperty("active", self._idea_filter == "all")
        self.filter_warn.setProperty("active", self._idea_filter == "warn")
        self.filter_danger.setProperty("active", self._idea_filter == "danger")
        self.filter_info.setProperty("active", self._idea_filter == "info")
        pairs = (
            (self.home_btn, "usage"),
        ...
        for btn, page in pairs:
            btn.set_active(self._page == page, force=True)
        for i, name in enumerate(THEMES):
            self.theme_btns[i].set_text(self.t(f"theme_{name}"))
            self.theme_btns[i].set_active(name == self._theme)
        ...
        for sec, btn in self.interval_btns:
            ...
            btn.setProperty("active", sec == self._interval)
        polish = (
            self.filter_all,
            self.filter_warn,
            self.filter_danger,
            self.filter_info,
            *(b for _, b in self.interval_btns),
        )
        for btn in polish:
    ...
    def _sync_nav(self, *, force: bool = False) -> None:
        ...
        for btn, page in (
            ...
            btn.set_active(self._page == page, force=force)

    def _set_filter(self, name: str) -> None:
        self._idea_filter = name
        self.filter_all.setProperty("active", name == "all")
        self.filter_warn.setProperty("active", name == "warn")
        self.filter_danger.setProperty("active", name == "danger")
        self.filter_info.setProperty("active", name == "info")
        for btn in (self.filter_all, self.filter_warn, self.filter_danger, self.filter_info):
            btn.style().unpolish(btn)
            btn.style().polish(btn)
    ...
    def _card(self, provider: ProviderUsage) -> dict:
        ...
        plan.setAlignment(Qt.AlignRight | Qt.AlignTop)
        pct = QLabel()
        pct.setObjectName("leftPct")
        pct.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
        right.addWidget(plan, 0, Qt.AlignRight)
        right.addWidget(pct, 0, Qt.AlignRight)
    ...
    def _got_icon(self, name: str, data: bytes, mark: QLabel) -> None:
        ...
        if not pix.loadFromData(data):
            return
        dest = _cache_dir() / f"{name.lower().replace(' ', '_')}.png"
    ...
    def _fill_ideas(self) -> None:
        ...
        self.stat_chats.set_values(str(report.chats), self.t("stat_l_chat"))
        ...
        rows: list[tuple[str, Finding | str, str, str]] = []
        stamp = datetime.now().strftime("%d.%m %H:%M")
        for item in report.findings:
            kind = _idea_kind(item.code)
            rows.append((kind, item, item.snippet, stamp))
        if report.mcps:
            rows.append(("info", "mcp", ", ".join(report.mcps), stamp))
        if report.skills:
            rows.append(("info", "skill", ", ".join(report.skills), stamp))
        shown = [
            r
            for r in rows
            if self._idea_filter == "all"
            or (self._idea_filter == "warn" and r[0] == "warn")
            or (self._idea_filter == "danger" and r[0] == "danger")
            or r[0] == self._idea_filter
        ]
        if not shown:
            self.ideas_layout.addWidget(self._idea_label(self.t("ideas_none"), "ideaBody"))
        for kind, payload, extra, when in shown:
            if isinstance(payload, Finding):
                self.ideas_layout.addWidget(
                    self._idea_card(
                        ...
                        f"{self.t('ideas_fix')}: {self.t(f'issue_{payload.code}_f')}",
                        payload.snippet,
                    )
                )
            else:
                self.ideas_layout.addWidget(
                    self._idea_card(kind, self.t("ideas_info"), when, extra, "", "", extra)
                )
        self.ideas_layout.addStretch(1)
    ...
    def _idea_card(
        ...
        look.clicked.connect(lambda: QGuiApplication.clipboard().setText(copy))
        foot.addWidget(look)
        box.addLayout(foot)
    ...
    def _show_page(self) -> None:
        self.pages.setCurrentIndex({"usage": 0, "ideas": 1, "settings": 2}[self._page])
        self._sync_nav()
    ...
    def _gate_license(self) -> bool:
        ver = self._settings.value("license_accepted_ver", "")
        if ver is None:
            ver = ""
        if not isinstance(ver, str):
            ver = str(ver)
        if ver == LICENSE_DOC_VER:
            return True
    ...
    def hide_to_tray(self) -> None:
        ...
        if QSystemTrayIcon.isSystemTrayAvailable():
            self.tray.showMessage(
                self.t("title"), self.t("tray_hide"), QSystemTrayIcon.Information, 2500
            )
    ...
    def refresh(self) -> None:
        if not self._quota_access:
            ...
            self.global_error.setText(self.t("usage_need_quota"))
    ...
    def _apply(self, snap: UsageSnapshot) -> None:
        try:
            ...
            if self.provider_cards and [c["name"] for c in self.provider_cards] == names:
                ...
                self.cards_layout.addStretch(1)
            tips = []
            for provider in items[:4]:
                if provider.meters:
                    tips.append(f"{provider.name} {self.tx(provider.meters[0].remaining_text)}")
            if tips:
                self.tray.setToolTip(" · ".join(tips))
        except Exception:
    ...
    def _fill(self, card: dict, provider: ProviderUsage) -> None:
        ...
        card["reset"].setText(reset_bits[0] if reset_bits else "")
        card["reset"].setVisible(bool(reset_bits))
        ...
        if multi and usable:
            primary = usable[0]
            pct_text = (
                self.t("left").format(n=primary.remaining_percent)
                if primary.remaining_percent is not None
                else self.tx(primary.remaining_text)
            )
            card["pct"].setText(pct_text)
            card["pct"].setVisible(bool(pct_text))
        for i, meter in enumerate(meters):
            if meter.remaining_percent is None and not meter.remaining_text:
                continue
            if multi:
                ...
                bar = Bar(tall=True)
                bar.set_remaining(meter.remaining_percent)
                card["body"].addWidget(bar)
                pct_text = (
                    self.t("left").format(n=meter.remaining_percent)
                    if meter.remaining_percent is not None
                    else self.tx(meter.remaining_text)
                )
                card["pct"].setText(pct_text)
                card["pct"].setVisible(bool(pct_text))
...
def main() -> int:
    if sys.platform == "win32":
        try:
            ...
            ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("TokenTakip.App")
        except Exception:
            pass
