...
from overlay import LANGS, TEXTS, UsageOverlay, _fetch_flags_sync, _flag_cache_path, _flag_pix, _letter_pix, _logo_pix, _tone_for, _username, provider_pix  # noqa: E402
...
def main() -> int:
    ...
    check("Sürüm", f"v{VERSION}" in win.subtitle.text())
    ...
    win.agents_btn.click()
    app.processEvents()
    check("Ajanlar sayfası", win._page == "agents")
    check("Ajanlar etiketi", win.agents_btn.text.text() == "Ajanlar")
    win.settings_btn.click()
    app.processEvents()
    check("Ayarlar sayfası", win._page == "settings")
    check("Sayfa sayısı", win.pages.count() == 8)
    check("Eşik tonu", _tone_for(10, 40, 15) == "crit" and _tone_for(25, 40, 15) == "warn")
    ...
    win.set_lang("tr")
    win.set_lang("tr")
    win.theme_btns[1][1].click()
    app.processEvents()
    check("Tema frost", win._theme == "frost")
    check("Tema stil uygulanır", "#f8fbff" in win.panel.styleSheet() or "contentArea" in win.panel.styleSheet())
    win.set_theme("night")
    app.processEvents()
    app.processEvents()
